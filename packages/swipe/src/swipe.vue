<template>
    <div class="sdSwipe" ref="sdSwipe">
        <div class="sdSwipe_box" ref="sdSwipe_box" @touchstart.stop="touchStart($event)" @touchmove.stop="touchMove($event)" @touchend.stop="touchEnd($event)" :style="`transform: translate(${!vertical ? (-clientNum * (isLoop ? currentIndex + 1 : currentIndex)) : '0'}px , ${!vertical ? '0' : (-clientNum * (isLoop ? currentIndex + 1 : currentIndex))}px)`">
            <slot ref="slot"></slot>
        </div>
